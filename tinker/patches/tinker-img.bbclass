...
EOF
	...
	mcopy -i ${WORKDIR}/${BOOT_IMG} -s ~/build-yocto-4SBCs/tinker/debian_kernel/arch/arm/boot/zImage ::
	mcopy -i ${WORKDIR}/${BOOT_IMG} -s ~/build-yocto-4SBCs/tinker/debian_kernel/arch/arm/boot/dts/rk3288-miniarm.dtb ::
	mmd -i ${WORKDIR}/${BOOT_IMG} ::/extlinux
	mcopy -i ${WORKDIR}/${BOOT_IMG} -s ~/build-yocto-4SBCs/tinker/patches/rk3288.conf ::/extlinux/extlinux.conf
	mcopy -i ${WORKDIR}/${BOOT_IMG} -s ~/build-yocto-4SBCs/tinker/patches/hw_intf.conf ::
